...
function startMarketCron() {
 cron.schedule("*/30 * * * * *", async () => {
  ...
  // 🔹 0. Generate a new market
  try {
    const newMarket = await generateRandomMarket();
    console.log("🆕 New market generated:", newMarket?.question || "no question");
  } catch (err) {
    console.error("❌ Failed to generate new market:", err.message);
  }
